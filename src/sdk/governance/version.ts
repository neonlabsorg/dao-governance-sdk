...
export async function getGovernanceProgramVersion(connection: Connection, programId: PublicKey, env?: string): Promise<number> {
  ...
  try {
    ...
    if (programMetadataInfo) {
      const programMetadata = governanceAccountParser(ProgramMetadata)(programMetadataPk, programMetadataInfo) as ProgramAccount<ProgramMetadata>;
