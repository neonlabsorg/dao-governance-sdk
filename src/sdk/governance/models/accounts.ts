...
export class MintMaxVoteWeightSource {
  ...
  static FULL_SUPPLY_FRACTION = new MintMaxVoteWeightSource({ value: MintMaxVoteWeightSource.SUPPLY_FRACTION_BASE });
...
export class VoteType {
  ...
  static MULTI_CHOICE = (choiceCount: number) =>
    new VoteType({ type: VoteTypeKind.MultiChoice, choiceCount: choiceCount });
...
export function getTokenHoldingAddress(programId: PublicKey, realm: PublicKey, governingTokenMint: PublicKey): PublicKey {
...
export function getRealmConfigAddress(programId: PublicKey, realm: PublicKey): PublicKey {
...
export function getTokenOwnerRecordAddress(programId: PublicKey, realm: PublicKey, governingTokenMint: PublicKey, governingTokenOwner: PublicKey): PublicKey {
...
export class Proposal {
  ...
  timeToVoteEnd({ config: { maxVotingTime } }: Governance) {
    const unixTimestampInSeconds = Date.now() / 1000;
    const votingAt = this.votingAt?.toNumber() ?? 0;
    return this.isPreVotingState ? maxVotingTime : votingAt + maxVotingTime - unixTimestampInSeconds;
  }

  timeToHoldupEnd({ config: { minInstructionHoldUpTime } }: Governance, instructions: ProposalTransaction[] = []) {
    const unixTimestampInSeconds = Date.now() / 1000;
    const votingAt = this.votingCompletedAt?.toNumber() ?? 0;
  ...
  canWalletCancel(governance: Governance, tokenOwner: TokenOwnerRecord, wallet: PublicKey): boolean {
    ...
    return (tokenOwner.governingTokenOwner.equals(wallet) || !!tokenOwner.governanceDelegate?.equals(wallet));
  }
}
...
export function getSignatoryRecordAddress(programId: PublicKey, proposalKey: PublicKey, signatoryKey: PublicKey): PublicKey {
...
export function getVoteRecordAddress(programId: PublicKey, proposalKey: PublicKey, tokenOwnerRecordKey: PublicKey) {
...
export class InstructionData {
  ...
  static transform({ programId, data, keys }: TransactionInstruction): InstructionData {
    const accounts: AccountMetaData[] = keys.map(k => new AccountMetaData(k));
...
export function getProposalTransactionAddress(programId: PublicKey, programVersion: number, proposal: PublicKey, optionIndex: number, transactionIndex: number): PublicKey {
...
export function getProgramMetadataAddress(programId: PublicKey): PublicKey {
  const [signatoryRecordAddress] = PublicKey.findProgramAddressSync([Buffer.from('metadata')], programId);
  return signatoryRecordAddress;
}

export function getNativeTreasuryAddress(programId: PublicKey, governance: PublicKey): PublicKey {
...
export class Vector<T> {
  ...
  constructor(args: { length: number; items: number; data: T[] }) {
...
export class VestingSchedule {
  ...
  constructor(args: { releaseTime: BN, amount: BN }) {
